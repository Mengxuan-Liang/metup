...
router.get("/", async (req, res) => {
  ...
  const jsonGroup = groups.map((group) => {
    ...
    return {
      ...
      previewImage: jsonG.previewImage.length
        ? jsonG.previewImage.map((el) => el.url)
        : null,
    };
  });
...
const validateGroup = [
  ...
  check("privateState").isBoolean().withMessage("Private must be a boolean"),
...
router.post("/", requireAuth, validateGroup, async (req, res) => {
  const { name, about, type, privateState, city, state } = req.body;
  ...
  if (groupId) {
    ...
    if (group) {
      ...
      if (organizer === currentUser) {
        ...
        res.status(403).json({
          message:
            "You are not the organizer, you cannot add image for this group.",
        });
      }
    } else {
  ...
  if (groupId) {
    ...
    if (group) {
      ...
      if (organizer === currentUser) {
        ...
        if (privateState !== undefined) group.private = privateState; //when updating the private field(value of true or false), the value false is considered falsy, so the logic (if (private) group.private = private;) will skip updating the field if private is false.
        ...
        res.status(403).json({
          message: "You are not the organizer, you cannot edit this group.",
        });
      }
    } else {
      res.status(404).json({ message: "Group could't be found" });
  ...
  if (groupId) {
    ...
    if (group) {
      ...
      if (organizer === currentUser) {
        ...
        res.status(403).json({
          message: "You are not the organizer, you cannot delete this group.",
        });
      }
    } else {
      res.status(404).json({ message: "Group could't be found" });
...
router.get("/:groupId/venues", requireAuth, async (req, res) => {
  ...
  if (groupId) {
    ...
    if (!membership) {
      return res
        .status(404)
        .json({ message: "No membership between the user and group" });
    }
    if (group) {
      if (
        currentUser === group.organizerId ||
        ...
        res
          .status(403)
          .json({ message: "You can not access to this infomation" });
      }
    } else {
      res.status(404).json({
        message: "Group couldn't be found",
    ...
    if (group) {
      ...
      const formattedEvents = events.map((event) => {
        ...
        return {
          ...
          previewImage: eventData.previewImage.length
            ? eventData.previewImage.map((el) => el.url)
            : null,
...
const validateEvent = [
  ...
  check("endDate")
    .custom((value, { req }) => {
      //{ req } is used to access req.body.startDate. This allows to compare startDate with endDate directly during validation.
    ...
    if (groupId) {
      ...
      if (group) {
        ...
        if (membership && membership.status === "pending") {
          ...
          res
            .status(400)
            .json({ message: "User is already a co-host of the group" });
        } else {
              ...
              if (status === "member") {
                ...
                if (
                  ...
                  (currentUserMembership &&
                    currentUserMembership.status === "co-host")
                ) {
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        message: "An error occurred while trying to delete the membership",
      });
    }
  }
);
